...
from model.MyNet.net_component_Param_FLOPS_FPS import BasicConv, Encoder_Block, Differ_Enhance, Decoder_Block
...
class MyCDNet(nn.Module):
    def __init__(self):
        ...
        # self.encoder_block1_basicconv1 = nn.Conv2d(in_channels=32, out_channels=32,kernel_size=3,stride=1,padding=1)
        # self.encoder_block1_basicconv2 = nn.Conv2d(in_channels=32, out_channels=32,kernel_size=3,stride=1,padding=1)
...
net = MyCDNet()
net = net.cuda()
input_x1 = torch.randn(8,3,256,256).cuda()
input_x2 = torch.randn(8,3,256,256).cuda()
flops,params = profile(model=net,inputs=(input_x1,input_x2))
print(flops/1e6)
print(params/1e6)
# summary(net,input_data=(input_x1,input_x2))

# torch.cuda.synchronize()
# start = time.time()
# result = net(input_x1,input_x2)
# torch.cuda.synchronize()
# end = time.time()
