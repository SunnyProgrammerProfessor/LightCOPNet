...
from model.MyNet.net_component_ablation_T2SSAE import BasicConv, Encoder_Block, Differ_Enhance, Decoder_Block
...
class MyCDNet(nn.Module):
    def __init__(self):
        ...
        self.encoder_block1_basicconv1 = BasicConv(
            in_channels=32, out_channels=32)
        self.encoder_block1_basicconv2 = BasicConv(
            in_channels=32, out_channels=32)

        self.encoder_block2 = Encoder_Block(
            in_channels=32, out_channels=64)  # 8,64,128,128
        ...
        self.encoder_block3 = Encoder_Block(
            in_channels=64, out_channels=128)  # 8,128,64,64
        ...
        self.encoder_block4 = Encoder_Block(
            in_channels=128, out_channels=256)  # 8,256,32,32
        ...
        self.encoder_block5 = Encoder_Block(
            in_channels=256, out_channels=512)  # 8,512,16,16
        ...
        self.decoder_block1 = Decoder_Block(
            in_channels=512, out_channels=256)  # 8,256,32,32
        self.decoder_block2 = Decoder_Block(
            in_channels=256, out_channels=128)  # 8,128,64,64
        self.decoder_block3 = Decoder_Block(
            in_channels=128, out_channels=64)  # 8,64,128,128
        ...
        self.change_out = nn.Conv2d(
            in_channels=8, out_channels=1, kernel_size=7, stride=1, padding=3)

    def forward(self, x1, x2):
        # encoder
        x1_en1 = self.encoder_block1(x1)  # 8,32,256,256
        x1_en1 = self.encoder_block1_basicconv2(
            self.encoder_block1_basicconv1(x1_en1))  # 8,32,256,256
        x2_en1 = self.encoder_block1(x2)  # 8,32,256,256
        x2_en1 = self.encoder_block1_basicconv2(
            self.encoder_block1_basicconv1(x2_en1))  # 8,32,256,256
        x1_en2 = self.encoder_block2(x1_en1)  # 8,64,128,128
        x2_en2 = self.encoder_block2(x2_en1)  # 8,64,128,128
        differ_1 = self.differ_1_conv(
            torch.abs(x2_en2 - x1_en2))  # 8,64,128,128
        ...
        differ_2 = self.differ_2_conv(
            torch.abs(x2_en3 - x1_en3))  # 8,128,64,64
        ...
        differ_3 = self.differ_3_conv(
            torch.abs(x2_en4 - x1_en4))  # 8,256,32,32
        ...
        differ_4 = self.differ_4_conv(
            torch.abs(x2_en5 - x1_en5))  # 8,512,16,16
        # decoder
...
# net = MyCDNet().to("cuda:2")
# T1 = torch.randn(8,3,256,256).to("cuda:2")
# T2 = torch.randn(8,3,256,256).to("cuda:2")
