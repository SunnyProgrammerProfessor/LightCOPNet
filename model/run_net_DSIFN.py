...
from model.MyNet.dual_differ_channelspatial_attention_DSIFN import MyCDNet
...
def train_model(args, dataloaders, writer):
    ...
    device = torch.device("cuda:2")  # device:cuda
    ...
    for epoch_id in range(epoch_to_start, max_epochs):
        ...
        for batch_id, batch in enumerate(dataloaders['train']):
            ...
            if np.mod(batch_id, 10) == 1:  # 每隔10steps输出一次
                ...
                logger.write("\n")
            if np.mod(batch_id, 500) == 1:  # LEVIR-CD数据集batch_size=16时,batch_id<500,因此每一个epoch可视化一次结果
