...
from model import loss_ablation_T2SSAE
import numpy as np
from model.visualization import make_numpy_grid, vis_norm
import matplotlib.pyplot as plt
from model.MyNet.ablation_onlySkipFusionEnhance import MyCDNet
...
def train_model(args, dataloaders, writer):
    ...
    device = torch.device("cuda:3")  # device:cuda
    ...
    # 定义损失函数
    if args.loss == "ce":
        _pxl_loss = loss_ablation_T2SSAE.cross_entropy
    elif args.loss == "bce":
        _pxl_loss = loss_ablation_T2SSAE.binary_ce
    else:
    ...
    for epoch_id in range(epoch_to_start, max_epochs):
        ...
        for batch_id, batch in enumerate(dataloaders['train']):
            ...
            net_loss = _pxl_loss(net_pred, image_gt)
            ...
            if np.mod(batch_id, 10) == 1:  # 每隔10steps输出一次
                ...
                logger.write("\n")
            # if np.mod(batch_id, 500) == 1:  # LEVIR-CD数据集batch_size=4时,batch_id<500,因此每一个epoch可视化一次结果
